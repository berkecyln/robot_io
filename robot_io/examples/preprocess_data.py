...
from robot_io.utils.utils import (
    depth_img_from_uint16,
    quat_to_euler,
    to_relative_all_frames,
)
...
def load_data(data):
    ...
    robot_obs = np.concatenate(
        [tcp_pos, tcp_orn, [gripper_width], joint_positions, [gripper_action]]
    )
...
def process_data(recording_dir, i, cam_num):
    ...
    rgb_static_list, depth_static_list, rgb_gripper, depth_gripper = load_imgs(
        data_cur, cam_num
    )
    depth_static_list = [
        depth_img_from_uint16(depth_static, max_depth=4)
        for depth_static in depth_static_list
    ]
    depth_gripper = (
        depth_img_from_uint16(depth_gripper, max_depth=4)
        if depth_gripper is not None
        else None
    )
    if (
        np.linalg.norm(rel_action["world_frame"][3:6]) > 1.5
        or np.linalg.norm(rel_action["world_frame"][:3]) > 1.25
    ):
        ...
        rel_action = compute_rel_action(
            tcp_pos, tcp_orn, next_tcp_pos, next_tcp_orn, curr_gripper_action
        )
        if (
            np.linalg.norm(rel_action["world_frame"][3:6]) > 1.5
            or np.linalg.norm(rel_action["world_frame"][:3]) > 1.25
    ...
    curr_action = np.concatenate(
        [curr_action[0], quat_to_euler(curr_action[1]), [curr_action[2]]]
    )
    if cam_num > 1:
        depth_static_dict = {
            f"depth_static_{i}": depth_static
            for i, depth_static in enumerate(depth_static_list)
        }
        rgb_static_dict = {
            f"rgb_static_{i}": rgb_static
            for i, rgb_static in enumerate(rgb_static_list)
        }
        save_data = {
            "actions": curr_action,
